...
# Generally Future values are influenced by past happening. It might be a trend or Seasonality 
# or both or cyclic. So I am here, depends upon the curve happened over time I will find out the 
...
#here we can see no.of rows and columns.

#data types are not correct. we should change at data cleaning part
...
# 
# . with help of standard deviation, we can find those outliers in which category they fall under 2nd or 3rd+ sd
# 
# # ____
...
#By using the panda's plot we can observe the behavior of the curve happened over time.
...
#here with the help of decomponents of series, we can know if series has trend or seasonality or both
...
# The Series has seasonality and variance not trend.
# 
# 2. However, we need to find which type is suitable for the Series by checking the resid
# 
# # ____
...
# ### Observation :
# 
# #### With auto ARIMA we can get the autoregression and Moving Average correlation points that are called PACF and ACF. The Auto ARIMA itself works meaningfully.  However It is depending on the parameter values. some times it does work like it gives an irrelative predicted curve it depends on the series to different series. that time we have to make it manually I mean with ARIMA Model.
# 
# ####  Now coming to the point, The order is (2,0,0) and Seasonal Order (2,0,1,12) and its AIc is 332 which is a very low value and we require it. except for AR2 and ARS24 remain Ar and Ma got less than a 0.05 value which means good and getting best observations.
# 
# 
# # _____
...
# D is 1
# 
# I tried all, however order(11,1,6) and seasonal order(0,0,2,12) has
# given a better root mean square error which is smaller than auto ARIMA's root mean square error. along with I kept confidence interval alpha =0.05 which gives
...
# ### Finally, we could forecast the future values meaning fully( Less RMSE) Gemerally, we can predict all kinds of series. however past circles play a vital role for forecasting. we are depending on the past transaction then we can predict it.
